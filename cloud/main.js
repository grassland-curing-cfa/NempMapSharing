...
Parse.Cloud.define("sendMapSharingEmailToUsers", function(request, response) {
	...
	var distribution_email_list = "";
	var distribution_email_tbl_html = "";

	// Find the DISTRIBUTION_EMAIL_GROUP and DISTRIBUTION_EMAIL_TABLE_HTML settings
	var queryAppSettings = new Parse.Query("APP_SETTINGS");
	queryAppSettings.first().then(function(appSettings) {
		...
		return queryMapSharingClass.find();
	}, function(error) {
	    console.error("APP_SETTINGS table lookup failed");
	    response.success(false);
			...
			var htmlString = 
						...
						'<td><p style="color:#C00000; font-weight: bold;">Research and Development Team</p><p>Fire Risk, Research and Community Preparedness - 8 Lakeside Drive, Burwood East, Victoria, 3151</p>' + 
